Được. Ta sẽ xây **Fetcher cho app crawl truyện chữ** theo hướng **DDD + SOLID**, dùng **`httpx` sync**, hỗ trợ:

* Proxy pool
* Xoay vòng proxy
* Xoay vòng `User-Agent`
* Kiểm tra proxy sống trước khi fetch
* Timeout
* Retry
* Phân loại lỗi
* Không để Domain phụ thuộc `httpx`
* Có thể test hoàn toàn bằng fake/mock
* Sau này dễ thay `httpx sync` → `httpx async`
* Có thể tích hợp với `Crawler`, `Parser`, `Repository`, `Unit of Work` mà bạn đã học

Ta **không viết một class `Fetcher` khổng lồ**. Trước tiên thiết kế kiến trúc.
...
```text
Phần I — Architecture
1. Thiết kế kiến trúc Fetcher
2. Domain Model cho Proxy
3. ProxyPool
4. User-Agent Pool
5. HttpClient abstraction
6. Proxy Health Checker
7. Fetcher cơ bản
8. Proxy rotation
9. UA rotation
10. Retry
...
28. Logging
29. Metrics
30. Concurrent fetching
31. Proxy failure strategy
32. Crawl session
33. Request fingerprint
34. Final Fetcher architecture
```
