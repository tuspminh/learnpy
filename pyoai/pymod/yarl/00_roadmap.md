Được. Tôi sẽ dạy **`yarl`** theo kiểu **từng bước → bài bản → có code hoàn chỉnh → chạy/test được ngay**, và đặc biệt liên hệ với **app crawler truyện chữ** mà bạn đang xây dựng.

`yarl` rất đáng học nếu bạn làm crawler vì nó cung cấp **URL object immutable**, xử lý URL/query/path tốt hơn việc nối chuỗi thủ công, và đặc biệt phù hợp với hệ sinh thái `aiohttp`.

## Roadmap học `yarl`

### Phần I — Nền tảng URL

1. **Giới thiệu `yarl` và vì sao không nên xử lý URL bằng string**
2. **Tạo URL với `URL()`**
3. **Các thành phần của URL**
   ...
   * fragment
4. **URL immutable**
5. **`str(URL)` và `repr(URL)`**
6. **URL tương đối và URL tuyệt đối**
7. **Join URL với `/`**
8. **Path parameters**
9. **Query string cơ bản**
10. **Query nhiều giá trị**

### Phần II — URL manipulation
