Được. Với mục tiêu **Novel Crawler** của bạn, mình đề xuất học `curl_cffi` theo hướng **từ API cơ bản → HTTP client → TLS/impersonation → proxy/cookie/header → retry → tích hợp vào Fetcher DDD/SOLID**, thay vì chỉ học từng hàm rời rạc.

`curl_cffi` đặc biệt đáng học vì nó cung cấp Python bindings cho `libcurl` và hỗ trợ **TLS/browser impersonation**, nên rất phù hợp để nghiên cứu HTTP client cho crawler.

## Roadmap học `curl_cffi`

### Phần I — Cơ bản

1. **Cài đặt & request đầu tiên**
2. `get()` và `post()`
3. `Response`
4. Status code
5. Headers
6. Text / content / encoding
7. JSON response
8. Query parameters
9. Request body
10. Timeout

### Phần II — Request chuyên sâu
...
### Phần III — curl_cffi đặc trưng

21. `impersonate`
22. Browser TLS fingerprint
23. Chrome / Firefox impersonation
24. `ja3`
25. `akamai`
26. HTTP/2
27. HTTP/3
28. So sánh `requests` / `httpx` / `curl_cffi`
29. Khi nào nên dùng `curl_cffi`
30. Các giới hạn và trade-off

### Phần IV — Proxy & crawler

31. HTTP proxy
32. HTTPS proxy
33. Proxy authentication
34. Proxy rotation
35. User-Agent rotation
36. Cookie rotation
37. Timeout + retry
38. HTTP error handling
39. Proxy health check
40. Fetch pipeline

### Phần V — Production Fetcher

41. Thiết kế `HttpClient` interface
42. `CurlCffiHttpClient`
43. Dependency Injection
44. Fake HTTP client để test
45. Mapping `curl_cffi.Response` → `HttpResponse`
46. Exception mapping
47. Retry policy
48. Proxy provider
49. User-Agent provider
50. Production `Fetcher`

### Phần VI — Tích hợp Novel Crawler
...
Điểm quan trọng là **không để `curl_cffi` lan vào Domain/Application**. Nó chỉ nằm ở Infrastructure, tương tự cách trước đây ta giữ `httpx` trong Infrastructure.
