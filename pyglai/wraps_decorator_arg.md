...
from functools import wraps
# Tầng 1: Nhận tham số của Decoratordef phan_quyen(quyen_yeu_cau):
...
@phan_quyen("admin")def xoa_nguoi_dung(user_role, user_id):
    return f"Đã xóa thành công người dùng {user_id}."

@phan_quyen("editor")def viet_bai(user_role, tieu_de):
