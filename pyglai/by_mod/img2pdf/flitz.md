...
# 1. Đường dẫn file PDF đầu vàopdf_path = "document.pdf"
# 2. Mở tài liệu PDFdoc = fitz.open(pdf_path)
# 3. Thiết lập độ nét (DPI). 300 DPI là tiêu chuẩn cho ảnh sắc nétzoom = 300 / 72  # 72 là DPI mặc định của PDFmatrix = fitz.Matrix(zoom, zoom)

print(f"Bắt đầu chuyển đổi... Tổng số trang: {len(doc)}")
# 4. Lặp qua từng trang và lưu thành file JPGfor i, page in enumerate(doc):
